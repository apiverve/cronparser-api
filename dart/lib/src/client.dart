...
class CronparserValidationException implements Exception {
  final List<String> errors;

  CronparserValidationException(this.errors);

  @override
  String toString() => 'CronparserValidationException: ${errors.join("; ")}';
}
...
///
/// For more information, visit: https://apiverve.com/marketplace/cronparser?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [expression] (required) - The cron expression to parse (5-field or 6-field format)
class CronparserClient {
  final String apiKey;
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    'expression': ValidationRule(type: 'string', required: true),
  };

  CronparserClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/cronparser',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [CronparserValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw CronparserValidationException(errors);
    }
  }

  /// Execute a request to the Cron Expression Parser API.
  ///
  /// Parameters are validated before sending the request.
  Future<CronparserResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw CronparserException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return CronparserResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw CronparserException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw CronparserException('Resource not found');
      } else {
        throw CronparserException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is CronparserException) rethrow;
      throw CronparserException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Cron Expression Parser API client.
class CronparserException implements Exception {
  final String message;

  CronparserException(this.message);

  @override
  String toString() => 'CronparserException: $message';
}
